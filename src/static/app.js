document.addEventListener("DOMContentLoaded", () => {
            ...
            <ul class="participants-list">
              ${details.participants.length
                ? details.participants
                    .map(
                      (p) =>
                        `<li class="participant-item"><span class="participant-email">${p}</span><button class="participant-delete" data-email="${p}" data-activity="${name}" aria-label="Remove ${p}">✖</button></li>`
                    )
                    .join("")
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    const ok = window.confirm(`Remove ${email} from ${activity}?`);
    if (!ok) return;
    ...
    try {
      ...
      if (response.ok) {
        messageDiv.textContent = result.message;
        messageDiv.className = "success";
        // Refresh the activity list to reflect removal
        fetchActivities();
      } else {
